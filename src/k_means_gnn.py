...
class k_means_gnn(pyg.nn.conv.MessagePassing):
    def __init__(
        ...
        for _ in range(num_layers - 2):
            layers.append(pyg.nn.conv.GCNConv(hidden_channels, hidden_channels))
    ...
    def assign_to_clusters(self, embeddings):
        cluster_distances = t.norm(self.centres - t.unsqueeze(embeddings, 1), dim=2)
    ...
    def cluster_distances(self, assignments, embeddings):
        ...
        dist_to_assigned_centre = t.gather(self.centres, 0, gather_indices) - embeddings
    ...
    def calculate_centres_update(self, assignments, distances, gamma=0.01):
        ...
        update = -(
            gamma / t.unsqueeze(cluster_counts[mask], dim=1) * sum_of_distances[mask]
        )
        return mask, update
