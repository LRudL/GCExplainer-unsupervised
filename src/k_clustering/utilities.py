...
def load_syn_data(dataset_str):
    ...
    ## I ADDED THIS
    elif dataset_str == "local_BA_Shapes":
...
def prepare_syn_data(G, labels, train_split, if_adj=False):
    ...
    if if_adj:
        edges = torch.tensor(
            nx.to_numpy_matrix(G), requires_grad=True, dtype=torch.float
        )
...
def prepare_real_data(graphs, train_split, batch_size, dataset_str):
    ...
    print()
    print(
        f"Class split - Training 0: {train_zeros} 1:{train_ones}, Test 0: {test_zeros} 1: {test_ones}"
    )
...
def plot_activation_space(
    data, labels, activation_type, layer_num, path, note="", naming_help=""
):
    rows = len(data)
...
def get_top_subgraphs(
    top_indices, y, edges, num_expansions, graph_data=None, graph_name=None
):
    graphs = []
    color_maps = []
    labels = []
...
def get_node_distances(clustering_model, data):
    if isinstance(clustering_model, AgglomerativeClustering) or isinstance(
...
def plot_clusters(
    ...
    fig.suptitle(
        f"{clustering_type} Clustered {data_type} Activations of Layer {layer_num} {note}"
    )

    for i in range(k):
        scatter = ax.scatter(
            data[labels == i, 0], data[labels == i, 1], label=f"Cluster {i}"
        )
...
def plot_samples(
    ...
    fig.suptitle(
        f"Nearest Instances to {clustering_type} Cluster Centroid for {reduction_type} Activations of Layer {layer_num}",
        y=1.005,
    )
    ...
    if graph_data is not None:
        ...
        fig2.suptitle(
            f"Nearest Instances to {clustering_type} Cluster Centroid for {reduction_type} Activations of Layer {layer_num} (by node index)",
            y=1.005,
        )
    ...
    for i, ax_list in zip(l, axes):
        ...
        for view in num_nodes_view:
            ...
            tg, cm, labels, node_labels = get_top_subgraphs(
                top_indices, y, edges, num_expansions, graph_data, graph_name
            )
            top_graphs = top_graphs + tg
            color_maps = color_maps + cm

        if graph_data is None:
            for ax, new_G, color_map, g_label in zip(
                ax_list, top_graphs, color_maps, labels
            ):
                nx.draw(new_G, node_color=color_map, with_labels=True, ax=ax)
                ax.set_title(f"label {g_label}", fontsize=14)
        else:
            for ax, new_G, color_map, g_label, n_labels in zip(
                ax_list, top_graphs, color_maps, labels, node_labels
            ):
                nx.draw(
                    new_G,
            ...
            for ax, new_G, color_map, g_label, n_labels in zip(
                axes2[i], top_graphs, color_maps, labels, node_labels
            ):
                nx.draw(new_G, node_color=color_map, with_labels=True, ax=ax)
                ax.set_title(f"label {g_label}", fontsize=14)
    ...
    if isinstance(clustering_model, AgglomerativeClustering):
        fig.savefig(
            os.path.join(
                path,
                f"{k}h_{layer_num}layer_{clustering_type}_{views}view.png",
            )
        )
    else:
        fig.savefig(
            os.path.join(
                path,
                f"{layer_num}layer_{clustering_type}_{reduction_type}_{views}view.png",
            )
        )
    ...
    if graph_data is not None:
        if isinstance(clustering_model, AgglomerativeClustering):
            fig2.savefig(
                os.path.join(
                    path,
                    f"{k}h_{layer_num}layer_{clustering_type}_{views}view_by_node.png",
                )
            )
        else:
            fig2.savefig(
                os.path.join(
                    path,
                    f"{layer_num}layer_{clustering_type}_{reduction_type}_{views}view_by_node.png",
                )
            )
...
def plot_dendrogram(data, reduction_type, layer_num, path):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    fig.suptitle(
        f"HC Dendrogram of {reduction_type} Activation Space of Layer {layer_num}"
    )

    dendrogram = hierarchy.dendrogram(
        hierarchy.linkage(data, method="average"),
        truncate_mode="lastp",
        ax=ax,
        leaf_rotation=90.0,
        leaf_font_size=14,
...
def plot_completeness_table(model_type, calc_type, data, path):
    ...
    ax.set_title(
        f"Completeness Score (Task Accuracy) for {model_type} Models using {calc_type}"
    )
    ax.axis("off")
    ax.table(
        cellText=data,
